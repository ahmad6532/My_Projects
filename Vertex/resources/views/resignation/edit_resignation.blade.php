            ...
            <div class="col-lg-10 user-form rounded">
                <div class="white-bg border" style="padding:15px;">
                    <div class="text-center">
                        <h1 class="text-heading_vt pb-2">Edit Resignation</h1>
                    </div>
                    <form action="{{ route('update.resignation') }}" method="POST" id="myForm">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <input type="hidden" name="id" value="{{ $resignation_data->id }}">
                                    ...
                                    <select title="Company" id="company_id" name="company_id" required class="form-control"
                                        ...
                                        @forelse($companies as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('company_id', $item->id == $resignation_data->company_id) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class=" mb-1 position-relative month-field_vt">
                                        <input type="text" name="notice_date" autocomplete="off" id="dateInput2"
                                            value="{{ old('notice_date', date('d-m-Y', strtotime($resignation_data->notice_date))) }}"
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="form-label">Resignation Date <span class="red"
                                            style="font-size:14px;">*</span></label>
                                    <div class=" mb-1 position-relative month-field_vt">
                                        <input type="text" name="resignation_date" autocomplete="off" id="dateInput1"
                                            value="{{ old('resignation_date', date('d-m-Y', strtotime($resignation_data->resignation_date))) }}"
                                            class="form-control" placeholder="Select Date">
                                        <i class="fontello icon-calander1"></i>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="textarea1" class="form-label">Reason<span class="red"
                                            style="font-size:14px;">*</span></label>
                                    <textarea class="form-control" id="textarea1" name="reason" rows="3" placeholder="Some Personal Reason.">{{ $resignation_data->reason }}</textarea>
                                </div>
                            </div>
                            <div class="col-md-12">
        ...
        function getBranches() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#branch_id').html('');
                    var storedId = '{{ $resignation_data->branch_id }}'.trim();
        ...
        function getEmployees() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var employee = '{{ $resignation_data->emp_id }}'.trim();
                    for (var i = 0; i < data.length; ++i) {
