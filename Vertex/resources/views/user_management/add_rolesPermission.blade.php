        ...
        <div class="card">
            ...
            <div class="user-content-detail">
                ...
                <div class="d-flex justify-content-end">
                    ...
                    <form action="{{route('save.roles')}}" id="saveRoleForm" method="post">
                        @csrf
                    <div class="px-1">
                        <button name="submit" type="submit" class="page-btn roles-btn">Save</button>
                    </div>
                </div>
                <div class="form-group">
                    <label for="title">Role Name<span class="red" style="font-size:22px;">*</span></label>
                    <input type="text" maxlength="20" required autocomplete="off" placeholder="Enter Role Name" value="{{old('role_name')}}" name="role_name" class="form-control">
                    @error('role_name')
                            ...
                            <tr>
                                ...
                                <td>
                                    <label class="switch-container">
                                        <input type="checkbox" id="all_{{$item->id}}" class="all_checkbox " onclick="check('{{$item->id}}')" name="all[{{$item->id}}]">
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                @if($item->id !== 5 && $item->id !== 1 && $item->id !== 2 && $item->id !== 3 && $item->id !== 25)
                                <td>
                                    <input type="radio" id="read_{{$item->id}}" name="read[{{$item->id}}]" class="radio-size cursor-pointer">
                                </td>
                                <td><input type="radio" id="write_{{$item->id}}" name="write[{{$item->id}}]" class="radio-size  cursor-pointer"></td>
                                @if($item->id !== 10 && $item->id !== 11 && $item->id !== 15 && $item->id !== 6 && $item->id !== 18)
                                <td><input type="radio" id="delete_{{$item->id}}" name="delete[{{$item->id}}]" class="radio-size cursor-pointer"></td>
                                @endif
                                @endif
                            </tr>
                            @endif
