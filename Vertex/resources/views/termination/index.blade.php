    ...
    <div class="Datatable-content-area mt-2 dataTable-section">
            ...
            <div class='row pt-2'>
                ...
                <div class="col-lg-2 col-md-2 px-1 mb-2">
                    <div class="d-flex justify-content-end">
                        <div>
                            @if ($user->haspermission(['termination-all','termination-write']))
                            <a href="{{ url('employee/add-termination') }}">
                                <button name="submit" type="submit" class="page-btn">Add Termination</button>
                            </a>
                            @endif
                        </div>
                    </div>
                ...
                {{-- <div class="col-lg-3 col-md-3 mb-3">
                    <form action="{{ route('termination.search.branch') }}" method="get" id="myForm">
                        <div class="form-group position-relative caret-holder px-1">
                            <select name="selectBranch" id="selectBranch" required class="form-control"
                ...
                <table id="table1"
                    ...
                    <thead>
                        <tr>
                            ...
                            <th>Status</th>
                            @if ($user->haspermission(['termination-all','termination-write','termination-delete']))
                            <th></th>
                            @endif
                        </tr>
                    </thead>
                    <tbody class="Listing_vt">
                        @foreach ($employees as $key => $item)
                            <tr>
                                ...
                                <td>
                                    {{ optional($item->designation)->name ?? 'N/A' }}
                                </td>
                                <td>{{ date('d-m-Y', strtotime($item->termination_date)) }}</td>
                                        ...
                                        <div class="btn-group dropdown-btn-group dropdown">
                                            ...
                                            <ul class="dropdown-menu form-action-menu" style="">
                                                ...
                                                <li class="checkbox-row pt-1 pl-1 hover-option_vt">
                                                    <a href="{{ route('edit.termination', ['id' => $item->id]) }}"
                                                ...
                                                @endif
                                                @if ($user->haspermission(['termination-all','termination-delete']))
                                                <li class="checkbox-row pt-1 pl-1 hover-option_vt">
                                                    <a onclick="delEmp('{{ route('delete.termination', [$item->id]) }}')"
                                                        class="action-content_vt">
                                                        <label for="toggle-tech-companies-1-col-2"
        ...
        <script type="text/javascript">
         $(document).ready(function() {
            ...
            <div class="col-lg-3 col-md-3 mb-3">
                ...
                $('#table1_filter').before(customSelect);
        ...
        function delEmp(url) {
            ...
            function changeStatus(id, status) {
                ...
                Swal.fire({
                    title: 'Confirm',
                    text: 'Are you sure you want to ' + statusText + ' Termination? ',
                    iconHtml: '<img src="{{ asset('assets/images/delete-alert.png') }}">',
                    showCancelButton: true,
                    reverseButtons: true,
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            method: 'get',
                            dataType: 'json',
                            url: '{{ route('change.termination.status') }}',
                            data: {
                                id: id,
                                status: status
                            },
            ...
            //                     var url = '{{ url('edit-termination/' . 'id') }}';
            //                     edit = url.replace('id', result[i].id);
            //                     var url = '{{ url('delete-termination/' . 'id') }}';
            ...
            //                     input += ' <td>' + (result[i].desgn ? (result[i].desgn.name ? result[i].desgn.name : 'N/A') : 'N/A') + '</td>';
            //                     input += ' <td>' + result[i].termination_date + '</td>';
            //                     input += ' <td>' + result[i].notice_date + '</td>';
            ...
            //                         '<a href="{{ url('edit-termination/') }}/' + result[i].id + '" class="action-content_vt">' +
