            ...
            <div class="col-lg-10 user-form rounded">
                <div class="white-bg border" style="padding:15px;">
                    <div class="text-center">
                        <h1 class="text-heading_vt pb-2">Edit Leave Request</h1>
                    </div>
                    <form  method="POST" action="{{ route('update.leave.request') }}">
                        @csrf
                        <div class="row">
                            <input type="hidden" name="id" value="{{$leave_detail->id}}">
                            <div class="col-md-6">
                                <div class="form-group position-relative arrow_vt">
                                    ...
                                    <select name="company_id" id="company_id" onchange="getBranches()"
                                        required class="form-control" style="appearance: none;">
                                        <option disable selected>Select Company</option>
                                        @forelse ($companies as $company)
                                            <option value="{{ $company->id }}" {{old('company_id',$leave_detail->company_id == $company->id)?'selected':''}}>{{ ucwords($company->company_name) }}
                                            </option>
                                        @empty
                                ...
                                <div class="form-group position-relative arrow_vt">
                                    ...
                                    <select id="leave_type" name="leave_type" required onchange="getRemainingLeaves()" 
                                        ...
                                        @forelse ($Leave_types as $type)
                                            <option value="{{$type->id}}" {{old('leave_type',$leave_detail->leave_type == $type->id)?'selected':''}}>{{ucwords($type->types)}}</option>
                                        @empty
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" value="" name="remaining_leaves" id="remaining_leaves">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="nameInput" class="form-label">Total Remaining Leaves</label>
                                    <input type="text" class="form-control" id="total_remaining_leaves">
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    <label class="form-label">From<span class="red"
                                        ...
                                        <div class=" mb-1 position-relative month-field_vt">
                                            <input type="text" name="from_date" id="dateInput1" onchange="changeDates()" value="{{old('from_date',date('d-m-Y',strtotime($leave_detail->from_date)))}}" class="form-control" placeholder="Select Date">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="form-label">To<span class="red"
                                            style="font-size:14px;">*</span></label>
                                    <div class=" mb-1 position-relative month-field_vt">
                                        <input type="text" name="to_date" id="dateInput2" onchange="changeDates()" value="{{old('to_date',date('d-m-Y',strtotime($leave_detail->to_date)))}}" class="form-control" placeholder="Select Date">
                                        <i class="fontello icon-calander1"></i>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="nameInput" class="form-label">Approved Days<span class="red"
                                            style="font-size:14px;">*</span></label>
                                    <input type="number" class="form-control" name="approved_days" id="approved_days" value="{{old('approved_days',$leave_detail->approved_days)}}" placeholder="Enter Days">
                                </div>
                            </div>
                            {{-- <div class="col-md-6">
                                <div class="form-group position-relative arrow_vt">
                                    ...
                                    <select name="approved_by" id="approved_by"
                                        required class="form-control" style="appearance: none;">
                                            ...
                                            <option value="CEO" {{old('approved_by',$leave_detail->approved_by == 'CEO')?'selected':''}}>CEO</option>
                                            <option value="Admin" {{old('approved_by',$leave_detail->approved_by == 'Admin')?'selected':''}}>Admin</option>
                                            <option value="PM" {{old('approved_by',$leave_detail->approved_by == 'PM')?'selected':''}}>Project Manager</option>
                                    </select>
                                    <i class="fontello icon-down-dir icon-color"></i>
                                </div>
                            </div> --}}
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="textarea1" class="form-label">Remarks<span class="red"
                                            style="font-size:14px;">*</span></label>
                                    <textarea class="form-control" name="remarks" id="textarea1" rows="3" placeholder="I want to apply leave for specific reason.">{{old('remarks',$leave_detail->remarks)}}</textarea>
                                </div>
                            </div>
                            <div class="col-md-12">
    ...
    function getBranches() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#branch_id').html('');
                var branch = '{{$leave_detail->branch_id}}'.trim();
                var html = '<option selected disabled>Select Branch</option>';
                for (var i = 0; i < data.length; ++i) {
                    var selected = data[i].id.toString() ==  branch? 'selected' : '';
    ...
    function getEmployees() {
        ...
        $.ajax({
            ...
            success: function(response) {
                var data = response.data;
                $('#emp_id').html('');
                var employee = '{{$leave_detail->emp_id}}'.trim();
                var html = '<option selected disabled>Select Employee</option>';
                for (var i = 0; i < data.length; ++i) {
                    var selected = data[i].id.toString() ==  employee? 'selected' : '';
