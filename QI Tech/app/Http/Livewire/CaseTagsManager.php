...
use App\Models\case_tags;
use App\Models\Headoffices\CaseManager\HeadOfficeCase;
use Illuminate\Support\Facades\Auth;
use Livewire\Component;

class CaseTagsManager extends Component
{
    public $case_default_tags,$case_id;
    ...
    public function mount($case_id){
        ...
        if($case_id){
            $this->case_default_tags = isset($case->tags) ? json_decode($case->tags,true) : [];
            if(count($this->case_default_tags) == 0){
                $firstTag = case_tags::where('head_office_id',$headOffice->id)->first();
                if(!isset($firstTag)){
                    $firstTag = new case_tags();
                    $firstTag->head_office_id = $headOffice->id;
                    $firstTag->case_id = $case->id;
                    $firstTag->name = 'High Risk';
                    $firstTag->save();
                }
            }
        }
        $this->all_tags = case_tags::where('head_office_id',$headOffice->id)->get();
    ...
    public function addNewTag(){
        $this->msg = null;
        $newCategory = new case_tags();
        $headOffice = Auth::guard('web')->user()->selected_head_office;
        $newCategory->head_office_id = $headOffice->id;
        $newCategory->case_id = $this->case_id;
    ...
    public function custom_visible($tag_id){
        $this->selected_tag = case_tags::find($tag_id);
    ...
    public function assignTag($tag_id){
        ...
        if($tag_id){
            $headOffice = Auth::guard('web')->user()->selected_head_office;
            if($this->case_id){
                $case = HeadOfficeCase::where('id',$this->case_id)->first();
                $case_tags = isset($case->tags) ? json_decode($case->tags,true) : [];
                if(!in_array($tag_id,$case_tags)){
                    $case_tags[] = $tag_id;
                }
                $case->tags = json_encode($case_tags);
                $case->save();
            }else{
                // $this->emit('categoryUpdated',$this->category_id);
            }
            $case = HeadOfficeCase::find($this->case_id);
            $this->case_default_tags = isset($case->tags) ? json_decode($case->tags,true) : [];
        }
    }
    public function updateTagName($tag_id){
        ...
        $updatedCategory = case_tags::find($tag_id);
    ...
    public function removeCategory($tag_id){
        ...
        $headOffice = Auth::guard('web')->user()->selected_head_office;
        $this->msg = null;
        $delTag = case_tags::find($tag_id);
        if($this->case_id){
            $cases = HeadOfficeCase::where('id',$this->case_id)->get();
            $filteredCases = [];
    ...
    public function removeTag($tag_id)
    {
        ...
        $case_tags = isset($case->tags) ? json_decode($case->tags, true) : [];
        $case_tags = array_filter($case_tags, function($tag) use ($tag_id) {
            return $tag != $tag_id;
        });
    
        // Reindex the array
        $case_tags = array_values($case_tags);
        $case->tags = json_encode($case_tags);
        $case->save();
    
        $this->case_default_tags = $case_tags;
    ...
    {
        return view('livewire.case-tags-manager');
    }
}
