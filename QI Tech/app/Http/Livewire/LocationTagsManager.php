...
class LocationTagsManager extends Component
{
    public $location_default_tags,$loc_id;
    ...
    public function mount($loc_id){
        ...
        if($loc_id){
            $this->case_default_tags = isset($location->tags) ? json_decode($location->tags,true) : [];
            if(count($this->case_default_tags) == 0){
                $firstTag = location_tags::where('head_office_id',$headOffice->id)->first();
                if(!isset($firstTag)){
                    $firstTag = new location_tags();
                    $firstTag->head_office_id = $headOffice->id;
                    $firstTag->location_id = $location->id;
                    $firstTag->name = 'High Risk';
                    $firstTag->save();
                }
            }
        }
        $this->all_tags = location_tags::where('head_office_id',$headOffice->id)->get();
    ...
    public function addNewTag(){
        $this->msg = null;
        $newCategory = new location_tags();
        $headOffice = Auth::guard('web')->user()->selected_head_office;
        $newCategory->head_office_id = $headOffice->id;
        $newCategory->location_id = $this->loc_id;
    ...
    public function custom_visible($tag_id){
        $this->selected_tag = location_tags::find($tag_id);
    ...
    public function assignTag($tag_id){
        ...
        if($tag_id){
            $headOffice = Auth::guard('web')->user()->selected_head_office;
            if($this->loc_id){
                $location = Location::where('id',$this->loc_id)->first();
                $location_tags = isset($location->tags) ? json_decode($location->tags,true) : [];
                if(!in_array($tag_id,$location_tags)){
                    $location_tags[] = $tag_id;
                }
                $location->tags = json_encode($location_tags);
                $location->save();
            }else{
                // $this->emit('categoryUpdated',$this->category_id);
            }
            $location = Location::find($this->loc_id);
            $this->case_default_tags = isset($location->tags) ? json_decode($location->tags,true) : [];
        }
    }
    public function updateTagName($tag_id){
        ...
        $updatedCategory = location_tags::find($tag_id);
    ...
    public function removeCategory($tag_id){
        ...
        $headOffice = Auth::guard('web')->user()->selected_head_office;
        $this->msg = null;
        $delTag = location_tags::find($tag_id);
        if($this->loc_id){
            $locations = Location::where('id',$this->loc_id)->get();
            $filteredCases = [];
            foreach($locations as $location){
                $location_tags = isset($location->tags) ? json_decode($location->tags,true) : [];
                if(in_array($tag_id,$location_tags)){
                    $filteredCases[] = $location;
                }
            }
        }
            if(count($filteredCases) !== 0){
    ...
    public function removeTag($tag_id)
    {
        ...
        $location_tags = isset($location->tags) ? json_decode($location->tags, true) : [];
        $location_tags = array_filter($location_tags, function($tag) use ($tag_id) {
            return $tag != $tag_id;
        });
    
        // Reindex the array
        $location_tags = array_values($location_tags);
        $location->tags = json_encode($location_tags);
        $location->save();
    
        $this->case_default_tags = $location_tags;
    
    }
    
    public function render()
    {
        return view('livewire.location-tags-manager');
    }
}
