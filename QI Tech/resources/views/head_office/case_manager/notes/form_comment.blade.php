<form method="post" @if (isset($edit_comment)) style="display:none" @endif
    class="rounded @if (isset($edit_comment)) cm_comment_edit_form_{{ $comment->id }} @endif cm_comment_form mt-1 shadow @if (isset($parent)) form_reply_{{ $parent->id }} @endif"
    action="{{ route('case_manager.save_comment') }}" enctype="multipart/form-data">
    ...
    <div class="new_comment_form cm_upload_box_with_model ps-0">
        ...
        <span class="inline-block links relative ps-2">
        ...
        <p spellcheck="true" class="inline-block cm_comment_box main-comment-textarea w-100 mt-2 remove-the-before" name="comment" required placeholder="Type notes here & drop files here"
        ...
        <button type="submit" name="submit" class="border-0 btn btn-info btn-submit inline-block mb-0 " style="width: 100px;border-top-right-radius:5px;"><i
                    ...
                    <div class="modal-body">
                        <div class="new_link_wrapper">
                            ...
                            {{-- <div class="form-inline mt-3">
                                <h5 class="fw-semibold">File Removal Reminder</h5>
                                <p class="fw-semibold">If this is a link to files stored externally (such as cloud
                                    storage), you can set a reminder to notify you to remove these files in line with
                                    your data retention policy when the retention period arrives.</p>

                                <div>
                                    <input type="checkbox" name="is_remind_me" id="is_remind_me"
                                        class=" is_remind_me form-check-input">
                            ...
                            <button data-dismiss="modal" type="button" class="btn btn-white add_link">Add Link</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        $('.cm_comment_box').each(function(index) {
    // Assign a unique ID to each element if it doesn't already have one
    if (!$(this).attr('id')) {
        $(this).attr('id', 'commentBox-' + index);
    }

    var elementId = $(this).attr('id');

    // Check if TinyMCE is already initialized on this element and destroy it if necessary
    if (tinymce.get(elementId)) {
        tinymce.get(elementId).remove(); // Remove the existing TinyMCE instance
    }

    // Initialize TinyMCE on the element
    tinymce.init({
        selector: '#' + elementId,
        plugins: 'quickbars textcolor link',
