@foreach(App\Models\NearMiss::$contributingFactors as $title=> $factors)
    <div data-list="{{$title}}" class="{{$title}} highlight contributing_factor_{{strtolower(str_replace(' ','_',$title))}}_section col-md-6
    @if(!isset($data)) 
    ...
    @endif"
    >
        <h5 class="text-info m-t-10" style="width:fit-content;">{{$title}}</h5>
    @foreach($factors as $field=>$label)
        ...
        @else
        <label class="checkbox {{str_replace(' ', '_', strtolower($label))}} @if($nearmiss && $nearmiss->$field) active @endif" 
            @if (!isset($data))
