<form method="post" @if (isset($edit_comment)) style="display:none" @endif
    ...
    action="{{ isset($address) ? route('head_office.address.save_comment') : route('head_office.contacts.save_comment') }}" enctype="multipart/form-data">
    ...
    <div class="new_comment_form cm_upload_box_with_model ">
        ...
        @if (isset($address))
            ...
            @else
            <input type="hidden" id="contact_id" name="contact_id" value="{{ $new_contact->id }}">
        @endif
...
<script>
    ...
    $(document).ready(function() {
        var contacts = @json($contacts_attr ?? []);
        var addresses = @json($addresses_attr ?? []);
        var users = @json($users_attr ?? []);

            if (typeof Tribute !== "undefined") {
                // Initial Tribute instance for selecting the type

                // Tribute instance for users
                var userTribute = new Tribute({
                    trigger: "@user",
                    ...
                    menuItemTemplate: function (item) {
                        return `<span class="user-item">${item.original.value}</span>`;
                    },
                    selectTemplate: function (item) {
                        return `<span class="tribute-item" data-id="${item.original.key}">@${item.original.value}</span>`;
                    },
                    menuItemLimit: 10,
                    allowSpaces: true,
                ...
                // Tribute instance for contacts
                var contactTribute = new Tribute({
                    trigger: "@contact",
                    values: function (text, cb) {
                        text = text.toLowerCase();
                        cb(contacts
                            .filter(contact => contact.name.toLowerCase().includes(text))
                            .map(contact => ({
                                key: contact.id,
                                value: contact.name
                            }))
                    ...
                    lookup: "value",
                    menuItemTemplate: function (item) {
                        return `<span class="contact-item">${item.original.value}</span>`;
                    },
                    selectTemplate: function (item) {
                        return `<span class="tribute-item" data-id="${item.original.key}">@${item.original.value}</span>`;
                    },
                ...
                // Tribute instance for addresses
                var addressTribute = new Tribute({
                    trigger: "@address",
                    values: function (text, cb) {
                        text = text.toLowerCase();
                        cb(addresses
                            .filter(address => address.name.toLowerCase().includes(text))
                            .map(address => ({
                                key: address.id,
                                value: address.name
                            }))
                    ...
                    lookup: "value",
                    menuItemTemplate: function (item) {
                        return `<span class="address-item">${item.original.value}</span>`;
                    },
                    selectTemplate: function (item) {
                        return `<span class="tribute-item" data-id="${item.original.key}">@${item.original.value}</span>`;
                    },
                ...
                var commentBoxes = document.querySelectorAll(".cm_comment_box");
                commentBoxes.forEach(function(box) {
                    userTribute.attach(box);
                    contactTribute.attach(box);
                    addressTribute.attach(box);
