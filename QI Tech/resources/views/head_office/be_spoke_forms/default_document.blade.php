<form method="post" action="{{route('default_documents.default_document.store')}}" class="cm_task_form">
    @csrf
    ...
    @endif
    @if(isset($document))
        <input type="hidden" name="default_document_id" value="{{$document->id}}">
    @endif
    <div class="modal fade file_upload_model " @if(isset($document)) id="default_document_form_{{$document->id}}"  @else id="default_document_form" @endif   tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
            <div class="modal-header text-center">
                <h4 class="modal-title text-info w-100"><p class="text-success"><i class="fa fa-tasks fa-2x"></i></p>Default Document</h4>
                ...
                <div class="form-group">
                    ...
                    <textarea spellcheck="true"  spellcheck="true" type="text" name="description" class="form-control" required>@if(isset($document)){{$document->description}}@endif</textarea>
                </div>
                ...
                {{-- <div class="form-group">
                    ...
                    <select type="text" name="assigned[]" style="width: 100%" class="form-control select2" required multiple>
                        ...
                        @foreach($head_office_users as $u)
                            <option @if(isset($document) && $document->hasAssignedUser($u->id)) selected  @endif value="{{$u->id}}">{{$u->user->name}}</option>
                        @endforeach
                    </select>
                </div> --}}
