...
@section('top-nav-title', 'Bespoke Form Questions')
@section('content')
{{-- <nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{route('head_office.be_spoke_form.index')}}">Bespoke Forms</a></li>
        <li class="breadcrumb-item"><a
                href="{{route('head_office.be_spoke_forms_templates.form_template',$stage->form_id)}}">Form -
                {{substr($stage->form->name,0,30)}} </a></li>
        <li class="breadcrumb-item"><a
                href="{{route('head_office.be_spoke_forms_templates.form_template',$stage->form_id)}}">Stage -
...
<div id="content">
    ...
    <div>
        ...
        Please add question in this group
        <form name="fields_form" method="post"
            action="{{route('head_office.be_spoke_forms_templates.form_stage_question.save',[$stage->id,$group_id])}}">
            @csrf
                <table class="table table-striped" id="dataTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>Type: {{ucwords(str_replace('_',' ',$q->question_type))}} <br>Name:
                                {{$q->question_name}}<br> Label: {{$q->question_title}}
                            ...
                            <td>
                                <a class="btn btn-info"
                                    href="{{route('head_office.be_spoke_forms_templates.form_stage_questions.edit',$q->id)}}"><i
                                        class="fas fa-edit"></i> Edit Question</a>
                                <a class="btn btn-info"
                                    href="{{route('head_office.be_spoke_forms_templates.form_stage_questions.action',$q->id)}}"><i
                                        class="fas fa-tasks"></i> Actions</a>
                                <a class="btn btn-danger delete_question"
                                    href="{{route('head_office.be_spoke_forms_templates.form_stage_questions.delete',['question_id'=>$q->id,'_token'=>csrf_token()])}}"><i
                                        class="fas fa-times"></i> Delete Question</a>
                            </td>
                        </tr>
