        ...
        @else
            <div
                ...
                @isset($record->form)
                    ...
                    <div
                        class="cm_case_status font-weight-bold d-flex align-items-center fw-semibold @if ($record->recorded_case->isArchive == true) text-info @elseif($record->recorded_case->status == 'waiting' && $record->recorded_case->requires_final_approval == true) text-warning @elseif($record->recorded_case->status == 'open') text-success @elseif($record->recorded_case->status == 'closed') text-danger @endif">
                        <i class="fa-solid fa-circle mx-2" style="font-size: 6px;margin-top:3px;"></i>
                        @if ($record->recorded_case->isArchived)
                            Archived
                        @elseif($record->recorded_case->status == 'waiting' && $record->recorded_case->requires_final_approval == true)
                            Final Approval
                        @else
                            {{ $record->recorded_case->status() }}
                        @endif
                    </div>
                ...
                @endif
                        ...
                        <h6 class=" p-0" style="color: #999;font-weight:normal">Created on <b>{{ $record->created_at->format('d/m/Y') }}</b> at
                            <b>{{ $record->created_at->format('h:i A') }}</b></h6>
                        @if (isset($record->recorded_case))
                            <a href="{{ route('case_manager.view', $record->recorded_case->id) }}" class="btn btn-light d-flex align-items-center gap-3 border" style="width: fit-content;color:#999;">View Case <i class="fa fa-arrow-right" style="font-size: 14px;"></i></a>
