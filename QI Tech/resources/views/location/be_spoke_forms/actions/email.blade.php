        ...
        <div class="form-group">
            <p for="email_attachment">Attachment 
                @if(isset($actionEmail) && !empty($actionEmail->email_attachment)): 
                    <a target="_blank" class="preview-email-attachment btn btn-success" href="{{route('be_spoke_forms_templates.form_stage_questions.email.attachment.view',$actionEmail->id)}}" >View Attachment</a>
                    <a class='delete-email-attachment btn btn-danger' href="{{route('be_spoke_forms_templates.form_stage_questions.email.attachment.delete',['action_id'=>$actionEmail->id,'_token'=>csrf_token()])}}">Remove Attachment</a>
                @endif
            </p>
