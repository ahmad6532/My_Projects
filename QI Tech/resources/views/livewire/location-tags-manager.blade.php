<div style="position: relative;" x-data>
    <div class="d-flex align-items-center gap-1 flex-wrap">
        @foreach ($case_default_tags as $index => $case_tag)
            @php
                $case_tag = \App\Models\location_tags::find($case_tag)
            @endphp
                @if (isset($case_tag))
                
                <button wire:key="{{ $case_tag->id }}" id="main-cat" type="button"  wire:click="toggleTagVisibility"  class="btn btn-outline-secondary custom-button rounded fw-normal d-flex align-items-center gap-2 case-tags" style="background: {{$case_tag->color}};color:{{$case_tag->text_color}}">
                    {!!$this->getSvg($case_tag->icon,$case_tag->icon_color)!!} {{$case_tag->name}} <i class="fa-solid fa-xmark mx-2 " style="margin-top: 2px;" wire:click.stop="removeTag({{$case_tag->id}})"></i>
                </button>
                @endif
        @endforeach
        ...
        <div class="tooltip-category-wrapper2">
            ...
            <div class="category-info-wrapper"> 
                ...
                <div class="category-btn-wrapper3">
                    ...
                    <div class="cat-input-parent " wire:key="{{ $index }}" style="align-items: center;height:40px;">
                        ...
                        <input wire:model.lazy='category_input.{{$tag->id}}' wire:blur='updateTagName({{$tag->id}})' class="input-name" style="height: 100%;"   />
                ...
                <div class="category-btn-wrapper">
                    ...
                    <Button type="button" style="background-color:{{$tag->color}};" class="placeholder-btn d-flex align-items-center gap-1 justify-content-center"  wire:click.prevent='assignTag({{$tag->id}})' >{!!$this->getSvg($tag->icon,$tag->icon_color)!!}{{ $tag->name}}</Button>
                    @endforeach
                </div>
                @endif
                <button wire:click='toggleEditMode' type="button" class="btn edit-btn mt-2"><i class="fa-solid fa-pencil"></i> {{$isEdit ? 'Apply' : 'Edit Tags'}}</button>
                @isset($msg)
