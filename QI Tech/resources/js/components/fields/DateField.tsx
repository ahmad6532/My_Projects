...
export const DateFieldFormElement: FormElement = {
    ...
    validate: (formElement: FormElementInstance, currentValue: string):boolean => {
        ...
        if(element.extraAttributes.required){
            ...
            // Ensure currentValue is not empty and is a valid date
            return currentValue.length > 0 && isValidDate;
        }
        return true;
    },
};
...
function PropertiseComponent({elementInstance}:{
    elementInstance: FormElementInstance}){
        ...
        useEffect(() => {
            if (found) {
                reset({id:found.id,label:found.extraAttributes?.label,helperText:found.extraAttributes?.helperText,required:found.extraAttributes?.required});
            }
        }, [element, reset]);
        // useEffect(() => {
