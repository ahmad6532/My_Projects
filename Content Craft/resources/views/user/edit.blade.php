...
@section('content')
    <form action="{{route('user.update',$userData->id)}}" id="editeUser" method="POST" class="w-100 mt-3">
        @csrf
        @method('PUT')
            ...
            <div class="w-50">
                <div class="form-outline mb-3 signup-input-div">
                    ...
                    <select name="managerId" value="{{$userData->managerId}}"  required class="form-control auth-form single-input">
                            <option value="{{$managerData->id}}">{{$managerData->firstName}} {{$managerData->lastName}}</option>
